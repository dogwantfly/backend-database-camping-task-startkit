...
SELECT 
    ...
    (SELECT u.name FROM "USER" u WHERE u.id = c.user_id) AS coach_name
FROM 
    "COURSE_BOOKING" cb
    JOIN "COURSE" c ON cb.course_id = c.id
WHERE 
    cb.user_id = (SELECT id FROM "USER" WHERE email = 'wXlTq@hexschooltest.io');
...
INNER JOIN "CREDIT_PACKAGE" p ON cp.credit_package_id = p.id
WHERE 
    cp.created_at >= '2024-11-01 00:00:00'
    AND cp.created_at < '2024-12-01 00:00:00'
GROUP BY 
    p.name
ORDER BY 
    銷售數量 DESC;
...
WHERE 
    ...
    AND cp.purchase_at < '2024-12-01 00:00:00';
...
FROM 
    "COURSE_BOOKING" cb
WHERE 
    cb.booking_at >= '2024-11-01 00:00:00'
    AND cb.booking_at < '2024-12-01 00:00:00'
